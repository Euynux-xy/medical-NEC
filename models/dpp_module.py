...
class DPPModule(nn.Module):
    ...
    def forward(
        ...
        for i in range(T):
            ...
            if i == 0:
                eis = k_j / (torch.sqrt(di2s[batch_arange, j]).unsqueeze(-1) + 1e-8)
            else:
                cov = torch.einsum("tb,tbn->bn", cis[:i, batch_arange, j], cis[:i])  # (B, N)
                eis = (k_j - cov) / (torch.sqrt(di2s[batch_arange, j]).unsqueeze(-1) + 1e-8)
            cis[i] = eis
            di2s = di2s - eis.pow(2)
            di2s[batch_arange, j] = -float("inf")

        # select_idx (T, B) -> (B, T)
        select_idx = torch.sort(select_idx.t(), dim=1).values  # (B, k)
        batch_idx = torch.arange(B, device=device).unsqueeze(1).expand(-1, self.k)
        return visual_features[batch_idx, select_idx]
